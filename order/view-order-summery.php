...
if (isset($_GET) && $_GET['order'] && $_SESSION['RoleID'] != 1) {
	...
	if ($queryFindOrder && mysqli_num_rows($queryFindOrder) == 1) {
				...
				<div class="main-content bg-color-12192C" id="main-content">
					<div class="row justify-content-center px-5">
						<div class="col-sm-10 mb-5">
							<h3 class="text-white h1 mb-3"><a href="./"><i class="bx bx-arrow-back font-weight-lighter text-white"></i></a> Order Details</h3>
							...
							<div class="card rd-10 border-0">
											...
											<div class="col-auto w-25 p-0">
												<div class="d-flex flex-column">
													<div class="row justify-content-between m-0">
														<p class="card-text font-weight-bolder">Order Summary</p>
													</div>
													<div class="row justify-content-between m-0 p-0">
														<p class="m-0 p-0">Service cost: </p>
														<p class="font-weight-bold m-0 p-0">
															<?php echo "<i class='bx bx-rupee'></i>" .  number_format($row['ServciePriceOnPurchaseTime'], 2); ?>
														</p>
													</div>
													<div class="row justify-content-between m-0 p-0">
														<p class="m-0 p-0">Service fee: </p>
														<p class="font-weight-bold m-0 p-0">
															<?php $tax = number_format($row['PayableAmount'] - $row['ServciePriceOnPurchaseTime'], 2);
															echo "<i class='bx bx-rupee'></i>" . $tax; ?>
														</p>
													</div>
													<div class="row justify-content-between m-0 p-0">
														<p class="m-0 p-0 font-weight-bold">Total: </p>
														<p class="font-weight-bold m-0 p-0">
															<?php echo "<i class='bx bx-rupee'></i>" . number_format($row['PayableAmount'], 2); ?>
														</p>
													</div>
												</div>
											</div>
										<?php } else if ($sessRoleID == 2) { ?>
											<a class="font-weight-bold btn Btn1 border-0 m-2" href="./order-deliver.php?order=<?php echo $row['OrderID']; ?>">Deliver Order</a>
										<?php
										} ?>
									</div>
								</div>
								...
								<div class="row mx-5 no-gutters">
									<p class="h4 card-title">Order Requirement</p>
								</div>
								<?php
								if ($row['RequirementStatus'] == 'P') {
								?>
									<div class="row mx-5 no-gutters">
										<p class="h5 card-title">Reqiurement Status:
											<strong>
