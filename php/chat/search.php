...
$sql = "SELECT * FROM `tbluserpersonaldetails` WHERE NOT `UserID` = {$outgoing_id} AND (`FirstName` LIKE '%{$searchTerm}%' OR `LastName` LIKE '%{$searchTerm}%'OR `Username` LIKE '%{$searchTerm}%') ";
$output = "";
$query = mysqli_query($dbConn, $sql);
while ($row = mysqli_fetch_assoc($query)) {
    $sessionUserID = $_SESSION['UserID'];
    $userID = $row['UserID'];
    $sqlGetChatUserList = "SELECT `tblmessage`.`SenderID`,`tblmessage`.`ReceiveID` FROM `tblmessage` WHERE (`SenderID`={$sessionUserID} AND `ReceiveID`={$userID}) OR (`SenderID`={$userID} AND `ReceiveID`={$sessionUserID}) ORDER BY MessageID DESC";
...
if ($output == "") {
    $output = 'No messages are available. Once you send message they will appear here.';
}
echo $output;
